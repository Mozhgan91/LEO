...
from .constants import (CLIP_MEAN, CLIP_STD, IMAGENET_MEAN, IMAGENET_STD,
                        ...
                        SIGLIP_MEAN, SIGLIP_STD, SAM_STD, SAM_MEAN)
...
def build_transform(is_train, input_size, pad2square=False, normalize_type='imagenet'):
    # If pad2square is False, the image is passed through a series of transformations:
    # Convert the image to RGB if it's not already.
    # Resize the image to a fixed size (input_size by input_size pixels) using bicubic interpolation.
    # Convert the image to a PyTorch tensor.
    # Normalize the image using pre-specified mean and standard deviation values.
    # TODO create a transform function for SAM if necessary
...
def build_transform_sam():
    MEAN, STD = IMAGENET_MEAN, IMAGENET_STD
    transform_sam = T.compose({
        T.Lambda(lambda img: img.convert('RGB') if img.mode != 'RGB' else img),
        T.ToTensor(),
        T.Normalize(mean=MEAN, std=STD),
        T.Lambda(lambda img: preprocess_sam(img)),
    })

    return transform_sam

def build_sam_processor():
    processor = SamProcessor.from_pretrained("facebook/sam-vit-large")
    processor.preprocess = processor.__call__
...
def preprocess_sam(images, processor):
    ...
    for img in images:
        if img.mode != 'RGB':
            img = img.convert('RGB')
            #print(f"Converted Image : Size = {img.size}, Mode = {img.mode}")
        processed_image = processor(img, return_tensors="pt")        
        processed_image["pixel_values"] = processed_image["pixel_values"].squeeze(0)
...
def preprocess_phi3(
    ...
    print(input_ids.shape) #it is different based on text length [1, length]
    targets = input_ids.clone()
